...
from _00_spreadsheet_handle import (
    read_pickle,
    write_pickle,
    get_file_paths,
    get_file_paths_dict,
...
def main_median_filter():
    ...
    learning_file_paths = get_file_paths(
        learning_address + "/Spreadsheets", extension="*.csv"
    )
    ...
    for key, path in learning_file_paths_dict.items():
        ...
        learning_lhs[key] = (
            pd.read_csv(path, skiprows=2, usecols=range(3, 49, 3))
            .dropna(how="all")
            .values
        )
    ...
    for key in learning_file_paths_dict.keys():
        ...
        learning_lhs[key] = medfilt(learning_lhs[key], kernel_size=(median_window, 1))
